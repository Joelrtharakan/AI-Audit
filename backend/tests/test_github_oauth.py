...
def test_session_creation_and_safe_dict_never_exposes_token():
    ...
    session = store.create_session(
        ...
        user_id="12345",
        user_principal_name="auditor_jane",
        name="Jane Auditor",
        email="jane@company.com",
        avatar_url="https://github.com/avatars/jane.png",
        organization="acme-corp",
        copilot_enabled=True,
        plain_access_token=raw_token,
    )
...
def test_session_expiration_cleanup():
    ...
    session = store.create_session(
        auth_provider="github",
        user_id="999",
        user_principal_name="temp_user",
        name="Temp",
        email="temp@test.com",
        avatar_url="",
        organization="test-org",
        copilot_enabled=True,
        plain_access_token="ghp_test",
    )
...
async def test_logout_destroys_session():
    ...
    session = store.create_session(
        auth_provider="github",
        user_id="123",
        user_principal_name="user_to_logout",
        name="User",
        email="",
        avatar_url="",
        organization="acme",
        copilot_enabled=True,
        plain_access_token="ghp_xyz",
    )
...
def test_provider_switching_development_ollama_vs_production_copilot():
    # The factory now returns the single LiteLLM boundary; the selected
    # provider/model is on .config.
    from app.services.llm.providers.litellm_provider import LiteLLMProvider

    dev_provider = get_llm_provider("ollama")
    assert isinstance(dev_provider, LiteLLMProvider)
    assert dev_provider.config.provider == "ollama"

    prod_provider = get_llm_provider("github_copilot")
    assert isinstance(prod_provider, LiteLLMProvider)
    assert prod_provider.config.provider == "github_copilot"
    assert prod_provider.config.litellm_model.startswith("github_copilot_session/")
